...
" Chapel extentions
syn keyword chapelStatement       break return continue compilerWarning delete
syn keyword chapelStatement       new delete this these use except only require import
syn keyword chapelStatement       noinit init
syn keyword chapelStatement       as module yield compilerError zip
syn keyword chapelIntent          param type in out inout ref
syn keyword chapelStorageClass    const config export extern var
syn keyword chapelType            domain sparse subdomain range index imag complex int uint real bool
syn keyword chapelType            file bytes string opaque integral numeric enumerated
syn keyword chapelType            locale sync atomic single dmapped
syn keyword chapelType            owned shared borrowed unmanaged
syn keyword chapelType            nothing void
syn keyword chapelOperator        on reduce scan by align
syn keyword chapelStructure       class record union enum
syn keyword chapelStructure       proc iter cobegin begin local sync let select where
syn keyword chapelStructure       pragma inline with private public forwarding
syn keyword chapelStructure       prototype override lifetime
syn keyword chapelBoolean         true false
syn keyword chapelConditional     if then else
syn keyword chapelConstant        nil none
syn keyword chapelRepeat          while for do coforall forall in serial
syn keyword chapelLabel           when otherwise label
syn keyword chapelErrorHandling   throw throws try catch

" Folding
syn region scopeFold start="{" end="}" fold transparent

" The minimum and maximum operators in GNU C++
syn match chapelMinMax "[<>]?"

" Default highlighting
if version >= 508 || !exists("did_chapel_syntax_inits")
  if version < 508
    let did_chapel_syntax_inits = 1
    command -nargs=+ HiLink hi link <args>
  else
    command -nargs=+ HiLink hi def link <args>
  endif
  HiLink chapelCast               chapelStatement
  HiLink chapelErrorHandling      chapelStatement
  HiLink chapelOperator           Operator
  HiLink chapelStatement          Statement
  HiLink chapelIntent             StorageClass
  HiLink chapelStorageClass       StorageClass
  HiLink chapelType               Type
  HiLink chapelStructure          Structure
  HiLink chapelNumber             Number
  HiLink chapelBoolean            Boolean
  HiLink chapelConditional        Conditional
  HiLink chapelConstant           Constant
  HiLink chapelRepeat             Repeat
  HiLink chapelLabel              Label
  delcommand HiLink
endif

let b:current_syntax = "chapel"
